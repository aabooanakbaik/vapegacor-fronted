...
import { MapPin, CreditCard, CheckCircle, Loader2, Package, ArrowLeft, QrCode, Copy } from 'lucide-react';
          ...
          {step === 3 && (
            <div className="text-center py-16">
              ...
              <div className="glass-card rounded-2xl p-6 max-w-sm mx-auto border border-green-500/15 mb-6">
                ...
                <p className="font-heading font-bold text-2xl gradient-violet-text mb-6">Rp {total.toLocaleString('id-ID')}</p>
                
                <div className="border-t border-white/10 pt-6">
                  {paymentMethod === 'qris' ? (
                    <div className="text-center">
                      <p className="text-ghost text-sm font-medium mb-4">Scan QRIS ini untuk membayar</p>
                      <div className="bg-white p-4 rounded-xl inline-block mb-3">
                        <QrCode className="w-48 h-48 text-slate-900" />
                      </div>
                      <p className="text-gray-500 text-xs">Mendukung GoPay, OVO, Dana, ShopeePay, LinkAja, dll</p>
                    </div>
                  ) : (
                    <div className="text-left">
                      <p className="text-gray-400 text-sm mb-2">No. Rekening Virtual Account ({
                        paymentMethod === 'va_bca' ? 'BCA' : 
                        paymentMethod === 'va_mandiri' ? 'Mandiri' : 'BNI'
                      })</p>
                      <div className="flex items-center justify-between bg-black/30 p-3 rounded-lg border border-white/5">
                        <span className="font-mono text-lg text-ghost tracking-wider">
                          {paymentMethod === 'va_bca' ? '8077 3456 7890' : 
                           paymentMethod === 'va_mandiri' ? '8990 1234 5678' : '8810 9876 5432'}
                        </span>
                        <button 
                          onClick={() => {
                            navigator.clipboard.writeText(
                              paymentMethod === 'va_bca' ? '807734567890' : 
                              paymentMethod === 'va_mandiri' ? '899012345678' : '881098765432'
                            );
                      ...
                      <p className="text-gray-500 text-xs mt-3 text-center">Atas Nama: VapeGacor Official</p>
                    </div>
                  )}
                </div>
              </div>
